...
- `circle_owner` (TEXT, optional) → Owner name (set to user's first_name when created via app)
- `invite_link` (TEXT, optional) → Full invite URL (auto-generated by database trigger)
- `created_at` (TIMESTAMPTZ)
...
- Generated from circle name: lowercase, no spaces, combined
- Example: "My Circle" → "mycircle"
- If token exists, appends numeric suffix: "mycircle1", "mycircle2", etc.
- Continues until unique token is found

**Invite Link Generation**:
- Auto-generated by database trigger on INSERT/UPDATE
- Format: `{baseUrl}/invite-landing?token={invite_token}`
- Base URL defaults to production, can be updated via application code
...
**Files**:
- `app/create-circle/page.tsx` (server component)
- `app/create-circle/create-circle-form.tsx` (client component)
- `app/api/create-circle/route.ts` (API route)
- `app/home/hamburger-menu.tsx` (client component - navigation)

**Flow Diagram**:
```
User clicks hamburger menu → "Create new circle"
  ↓
[Client] Navigates to /create-circle
  ↓
[Server] create-circle/page.tsx:
  - Authenticates user
  - Fetches user's profile (for first_name)
  - Fetches all circles where circle_owner = user's first_name
  ↓
[Client] create-circle-form.tsx renders:
  - Form with circle name input
  - Table of previously created circles (if any)
  ↓
User enters circle name and clicks "Create Circle"
  ↓
[Client] POST /api/create-circle with {name}
  ↓
[Server] create-circle/route.ts:
  1. Validates circle name
  2. Generates invite_token from name (lowercase, no spaces)
  3. Checks for duplicate token, appends number if needed (mycircle1, mycircle2, etc.)
  4. Gets user's first_name from profiles
  5. Inserts circle with:
     - name: from form
     - invite_token: generated token
     - circle_owner: user's first_name
  6. Database trigger auto-generates invite_link
  7. Fetches circle again to get invite_link
  8. Adds creator as member (insert into circle_members)
  9. Returns circle data including invite_link
  ↓
[Client] Displays success message with shareable invite link
  ↓
[Client] User can copy invite link to share
```

**Key Design Decisions**:
- **Name-based tokens**: Makes invite links more readable and memorable
- **Auto-membership**: Creator is automatically added to circle_members
- **Database trigger**: invite_link is auto-generated, ensuring consistency
- **Duplicate handling**: Numeric suffix prevents conflicts while keeping names readable

---

### 5.7 SMS Notification Flow
...
**Data Fetched**:
- User profile
- All circle memberships (supports multi-circle)
- Current week
- User's reflection (if exists) for selected circle
- Unlock status for selected circle
- Mid-week join status
- All circle members and their profiles for selected circle
- Submitted reflections for current week (for member status) for selected circle

**Circle Selection Logic**:
- Reads `circleId` from URL search params
- Fetches all user's circle memberships
- Determines selected circle: from URL or most recently joined (default)
- All subsequent queries filter by selected `circleId`
...
**UI Elements**:
- Hamburger menu in top right (Menu icon) with dropdown:
  - "Review past reflections" → Links to `/review?circleId={selectedCircleId}`
  - "Create new circle" → Links to `/create-circle`
- Circle switcher (if user is in multiple circles) → Positioned at top center
...
**Member Status Logic**:
- Fetches all circle members with join timestamps for selected circle
- Fetches all submitted reflections for current week for selected circle
...
#### `app/home/hamburger-menu.tsx`
**Type**: Client Component

**Purpose**: Navigation menu in top-right corner of home page

**Features**:
- Hamburger icon (Menu from lucide-react) that toggles to X when open
- Dropdown menu with two options:
  - "Review past reflections" → Links to review page with current circleId
  - "Create new circle" → Links to circle creation page
- Closes on outside click or menu item selection
- Positioned absolutely at top-right (top-4 right-4 on mobile, top-6 right-6 on larger screens)
...
#### `app/create-circle/page.tsx`
**Type**: Server Component

**Purpose**: Circle creation page with form and table of user's created circles

**Data Fetched**:
- User profile (for first_name to identify owned circles)
- All circles where `circle_owner` matches user's first_name
- Ordered by `created_at` descending (newest first)

**UI Elements**:
- Back button to home
- Header: "Create New Circle"
- CreateCircleForm component (client component)
- Table of previously created circles showing:
  - Circle name
  - Created date
  - Invite link (clickable)

**Access Control**:
- Redirects to `/invite` if not authenticated
- Redirects to `/invite` if profile not found

---

#### `app/create-circle/create-circle-form.tsx`
**Type**: Client Component

**Purpose**: Form for creating new circles

**State Management**:
- `circleName`: Input value
- `loading`: Submission state
- `error`: Error messages
- `createdCircle`: Successfully created circle data (name, invite_link)
- `copied`: Copy-to-clipboard state

**Features**:
- Circle name input field
- "Create Circle" button (disabled when loading or empty)
- Success message with shareable invite link
- Copy-to-clipboard button for invite link
- Error display for validation/API errors

**API Integration**:
- POST to `/api/create-circle` with `{name}`
- Receives circle data including `invite_link`
- Displays invite link in read-only input with copy button
...
#### `/api/create-circle`
**File**: `app/api/create-circle/route.ts`

**Purpose**: Create a new circle and add creator as member

**Input**: `{name}` (circle name)

**Logic**:
1. Validates circle name (required, non-empty)
2. Authenticates user
3. Gets user's profile (for first_name)
4. Generates invite_token from name:
   - Lowercase, remove spaces, combine
   - Example: "My Circle" → "mycircle"
   - If duplicate, appends number: "mycircle1", "mycircle2", etc.
5. Inserts circle with:
   - `name`: from form input
   - `invite_token`: generated unique token
   - `circle_owner`: user's first_name
6. Database trigger auto-generates `invite_link`
7. Fetches circle again to get `invite_link` from database
8. Adds creator as member (insert into `circle_members`)
9. Returns circle data including `invite_link`
...
- Returns 500 if circle creation or membership insertion fails

**Why Name-Based Tokens?**: Makes invite links more readable and memorable (e.g., `/invite-landing?token=mycircle` vs random UUID)

**Why Auto-Membership?**: Creator should immediately be part of the circle they created
...
**Purpose**: Generate and update invite links for circles

**Functions**:

- `generateInviteLink(token: string, baseUrl?: string): string`
  - Generates invite link URL from token
  - Format: `{baseUrl}/invite-landing?token={token}`
  - Base URL priority:
    1. Provided `baseUrl` parameter
    2. `NEXT_PUBLIC_APP_URL` env variable
    3. `VERCEL_URL` env variable (with https:// prefix)
    4. Defaults to production URL: `https://rose-bud-thorn.vercel.app`

- `updateCircleInviteLink(supabase: any, circleId: string, token: string, baseUrl?: string): Promise<void>`
  - Updates `invite_link` column in circles table
  - Ensures invite link has correct base URL
  - Used when base URL changes or needs manual update

**Usage**:
- Called by API routes when creating/updating circles
- Database trigger also generates invite_link automatically
- Application code can override with correct base URL if needed
...
### 15.1 Multi-Circle Support (v2.0) ✅ IMPLEMENTED

**Status**: Fully implemented in v2.0

**Features Added**:
- Users can be members of multiple circles
- Circle switcher on home and review pages
- Reflection submission to all user's circles
- Per-circle unlock SMS notifications (includes circle name)
- Circle selection via URL parameter (`?circleId=...`)
- Default circle: most recently joined circle

**Database Changes**:
- Removed `UNIQUE(user_id)` constraint from `circle_members`
- Added `UNIQUE(user_id, circle_id)` constraint (allows multiple memberships)
- Migration: `20250121000018_allow_multiple_circles.sql`

**UI Changes**:
- Circle switcher component (`app/home/circle-switcher.tsx`)
- Hamburger menu for navigation (`app/home/hamburger-menu.tsx`)
- Circle creation page (`app/create-circle/page.tsx`)

**Key Implementation Details**:
- All queries filter by `circleId` from URL or default selection
- Reflection submission inserts into all user's circles
- Unlock SMS includes circle name in message
- Circle owner tracking via `circle_owner` field (user's first_name)

---

### 15.2 Circle Creation Feature ✅ IMPLEMENTED

**Status**: Fully implemented

**Features**:
- Users can create new circles from within the app
- Name-based invite tokens (lowercase, no spaces)
- Automatic duplicate handling (appends numeric suffix)
- Auto-membership for creator
- Table of user's created circles
- Shareable invite links

**Files**:
- `app/create-circle/page.tsx` - Circle creation page
- `app/create-circle/create-circle-form.tsx` - Creation form
...
- Migration: `20250121000023_allow_users_create_circles.sql`
